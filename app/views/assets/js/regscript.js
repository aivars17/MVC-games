...
function shuffle(){
	...
	for (var i = 0; i < 3; i++) {		
		...
		document.getElementById("dice"+[i]).src = "../app/views/assets/pic/"+skaiciai[i]+".png";
	}
...
function intoDB(){
	$.post( "/studijos/mvc-games/dice/inToDB/", 
		{ inToDB: wins.toFixed(2)}, function( data ) {
});
};
//duomenų išvalimas ir naujo žaidimo pradėjimas
function newGame(){
	for (var i = 0; i < 3; i++) {
		document.getElementById("dice"+[i]).src = "/studijos/mvc-game/app/views/assets/pic/6.png";
	}
